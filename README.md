# wgjail-quick
Wgjail-quick gives you the ability to create a "Wireguard jail", i.e. a network namespace where a program that is running inside that namespace can only see and use the one wireguard connection you have given it. There are two main reasons why you want to do this:
...
1. Has a similar syntax to wg-quick, because peope already use wg-quick and are used to that syntax.
...
## portforward
Forwards a port into the network namespace. You have to specify on which Adress to listen and which port to forward. Use 0.0.0.0 to listen everywhere.  
...
When you start this service, write the user as make sure you append the username that shall run the command after the '@' symbol, e.g. '`systemctl start wgjail-NAMESPACE-CMD@myuser.service`'. Result will be saved to the current directory (`./wgjail-NAMESPACE-CMD@.service`).  
...
`sudo wgjail-quick up <wireguard_config>.conf`  
`sudo wgjail-quick exec <wireguard_config>.conf <your_username> qbittorrent`  
When you're done, close the qbittorrent window, then:  
`sudo wgjail-quick down <wireguard_config>.conf`  
Done, very easy.
...
`wgjail-quick generate-jail-service <wireguard_config>.conf`  
`wgjail-quick generate-cmd-service <wireguard_config>.conf /usr/bin/qbittorrent-nox -d --webui-port=6969`  
`wgjail-quick generate-portforward-service <wireguard_config>.conf 0.0.0.0 6969` 
